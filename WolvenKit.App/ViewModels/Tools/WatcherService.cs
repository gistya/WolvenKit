...
partial class ProjectExplorerViewModel
{
    ...
    private CancellationTokenSource _deferredRefreshCts2 = new();
    ...
    DispatchedObservableCollection<FileSystemModel> InternalFileList => _fileList;
    DispatchedObservableCollection<FileSystemModel> InternalFileTree => _fileTree;
    Guid CompletionTimer = Guid.NewGuid();
    WatcherState _watcherState = WatcherState.NoProject;
    ...
    void _StartWatcher_AndLoadProject(Cp77Project project, Action? completion = null)
    {
    ...
    /// InternalFileList or InternalFileTree collections.
    /// </summary>
    void _UnwatchProject()
    {
        Suspend();
    ...
    private void Update(CancellationToken cancellationToken)
    {
        ...
        void Changed(FileSystemEventArgsWrapper e)
        {
            ...
            item.UpdateFileInfo();
            _gridGuard.ProjectUpdateFileInfo(item);
        }
        ...
        void Renamed(FileSystemEventArgsWrapper e)
        {
            ...
            foreach (var key in _fileLookup.Keys)
            {
                ...
                item.Rename(newName);
                _gridGuard.ProjectRename(item, oldFullName);
            }
        }
    ...
    private ConcurrentQueue<Func<Task>> _refreshQueue = new ConcurrentQueue<Func<Task>>();
    ...
    private async Task RefreshAfter(Func<Task> action)
    {
        ...
        if (BeginDeferredRefreshContext == null)
        {
            ...
            if (_refreshQueue.TryDequeue(out var nextAction))
            {
                await RefreshAfter(nextAction);
            }

            return;
        }

        RefreshCts(ref _deferredRefreshCts2!);
        ...
        try
        {
            ...
            await BeginDeferredRefreshContext(
                _deferredRefreshCts2.Token,
                action
            );
        }
        catch (OperationCanceledException)
            when (_deferredRefreshCts2.IsCancellationRequested)
        {
        ...
        {
            DispatcherHelper.DelayOnMainThread(async () =>
            {
                try
                {
                    _deferredRefreshCts2?.CancelAsync();
                }
                catch (ObjectDisposedException)
                {
                }

                _deferredRefreshCts2?.Dispose();
                _deferredRefreshCts2 = new CancellationTokenSource();
    ...
    private void RemoveModel(FileSystemModel model, long removedAt = 0)
    {
        ...
        if (InternalFileTree.Contains(model))
            InternalFileTree.Remove(model);
        if (InternalFileList.Contains(model))
            InternalFileList.Remove(model);
        ...
        _gridGuard.ProjectRemove(model);
    }
    ...
    private void RemoveChildModels(FileSystemModel model)
    {
        ...
        foreach (var subModel in children)
        {
            ...
            if (InternalFileList.Contains(subModel))
                InternalFileList.Remove(subModel);
        }
    }
    ...
    private void LoadModProjectFileStructure()
    {
        ...
        void Clear()
        {
            ...
            _fileLookup.Clear();
            InternalFileTree.Clear();
            InternalFileList.Clear();
        }
        ...
        Task.Run(() =>
        {
            ...
            DispatcherHelper.RunOnMainThread(() =>
            {
                ...
                {
                    InternalFileList.AddRange(flatListReturn);
                }

                InternalFileTree.AddRange((treeRoot != null)
                    ? treeRoot.Children
                    : Array.Empty<FileSystemModel>());

                // Project the freshly built domain tree onto the grid-bound clones.
                _gridGuard.ProjectReset(flatListReturn, treeRoot?.Children.ToList() ?? new List<FileSystemModel>());
    ...
    /// Called from both Stop and defensively from Start.
    /// </summary>
    ...
    private void BatchUpdate(CancellationToken cancellationToken)
    {
        ...
        void ApplyBatch(List<FileSystemEventArgsWrapper> batch, CancellationToken ct)
        {
            ...
            {
                InternalFileList.AddRange(created);
                if (InternalFileTree.Count < 3)
                {
                    InternalFileTree.AddRange(created);
                }

                await RefreshAfter(() => _gridGuard.ProjectAdd(created));
    ...
    private void OnFilesImported(FilesImportedMessage msg)
    {
        ...
        InternalFileList.AddRange(batch);
        _gridGuard.ProjectAdd(batch);
        _Resume();
    ...
    /// no-op.
    /// </summary>
    private async Task OnFilesMoved(FilesMovedMessage msg)
    {
    ...
    private void RenameModelInPlace(FileSystemModel model, string oldFullName, string newFullName)
    {
        ...
        _gridGuard.ProjectRename(model, oldFullName);
    }
